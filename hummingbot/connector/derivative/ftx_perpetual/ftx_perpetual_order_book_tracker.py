...
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_active_order_tracker import FtxPerpetualActiveOrderTracker
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_api_order_book_data_source import FtxPerpetualAPIOrderBookDataSource
...
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_order_book_tracker_entry import FtxPerpetualOrderBookTrackerEntry
...
class FtxPerpetualOrderBookTracker(OrderBookTracker):
    ...
    def __init__(
        ...
        super().__init__(
            data_source=FtxPerpetualAPIOrderBookDataSource(trading_pairs=trading_pairs),
        ...
        self._order_books: Dict[str, FtxPerpetualOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[FtxPerpetualOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, FtxPerpetualActiveOrderTracker] = defaultdict(FtxPerpetualActiveOrderTracker)
    ...
    async def _refresh_tracking_tasks(self):
        ...
        available_pairs: Dict[str, FtxPerpetualOrderBookTrackerEntry] = await self.data_source.get_tracking_pairs()
        ...
        for trading_pair in new_trading_pair:
            order_book_tracker_entry: FtxPerpetualOrderBookTrackerEntry = available_pairs[trading_pair]
